...
def get_preprocessing(X):
    ...
    Returns:
        X_preprocessed (pd.DataFrame): the preprocessed dataset
    """
...
def get_app_prediction(model_uri, X_customer):
    """This function requests a Flak app by sending the data of the customer
    to the app and receives the response of the app with the result of
    prediction.
    ...
    Returns:
        app_result (float): the Flask app response containing the prediction of
                            the model
    """

    headers = {'Content-Type': 'application/json'}
    json_data = X_customer.to_dict(orient='records')[0]

    # Request of the dashboard
    response = requests.request(
        method='GET', headers=headers, url=model_uri, json=json_data)
    if response.status_code != 200:
        raise Exception('Request failed with status {}, {}'.format(
            response.status_code, response.text))

    # The Flask app response
    app_response = response.json()
    app_result = app_response['result']
    return app_result


def get_application_status(app_result, credit_score_risk_threshold):
    """This function determines the application status (risk and decision)
    for a customer.

    Args:
        app_result (float): app response containing the model prediction
                            (probability)
...
def main():
    ...
    try:
        ...
        try:
            APP_URI = 'http://127.0.0.1:5000/predict'
            app_result = get_app_prediction(APP_URI, customer_dataset)
            status, situation, score = get_application_status(
                app_result, risk_threshold)
        ...
        infos_viz = customers_description(similar_customers_dataset)
        similar_customers_viz = infos_viz.drop([customer_index])
        ...
        if info_display == "Visualisations":
            ...
            if display_description == "Financial information":
                ...
                try:
                    if not financial_information_features:
                        ...
                        st.dataframe(infos_viz.set_index(
                            'Customer ID')[financial_information_features])
                        ax = infos_viz.set_index('Customer ID')[
                ...
                st.subheader("Gender")
                try:
                    gender_viz = infos_viz.groupby('Gender').count()
                ...
                try:
                    st.bar_chart(
                        data=infos_viz.filter(['Customer ID', 'Age (years)']),
                        x='Customer ID',
                        y='Age (years)',
                        sort='-Age (years)'
                    )
                ...
                try:
                    st.bar_chart(
                        data=infos_viz.filter(['Customer ID', 'Days employed']),
                        x='Customer ID',
                        y='Days employed',
                        sort='-Days employed'
                    )
                ...
                try:
                    family_status_viz = infos_viz.groupby(
                        'Family status').count()
                ...
                try:
                    business_segment_viz = infos_viz.groupby(
            ...
            try:
                ...
                for id in list(similar_customers_viz['Customer ID']):
                    similar_customer_index = X.loc[
                        X['SK ID CURR'] == id].index[0]
                    similar_customer_dataset = X_preprocessed.loc[
                        [similar_customer_index]]
                    APP_URI = 'http://127.0.0.1:5000/predict'
                    app_result = get_app_prediction(
                        APP_URI, similar_customer_dataset)
                    status, situation, score = get_application_status(
                        app_result, risk_threshold)
        ...
        elif info_display == "Customer data":
